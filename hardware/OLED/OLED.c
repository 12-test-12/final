...
void OLED_W_SCL(uint8_t BitValue) {
  ...
  GPIO_WriteBit(OLED_PORT, OLED_SCL_PIN, (BitAction)BitValue);
...
void OLED_W_SDA(uint8_t BitValue) {
  ...
  GPIO_WriteBit(OLED_PORT, OLED_SDA_PIN, (BitAction)BitValue);
...
static uint8_t OLED_StringEqual(const char *String1, const char *String2) {
 ...
 */
uint8_t OLED_IsInAngle(int16_t X, int16_t Y, int16_t StartAngle,
                       int16_t EndAngle) {
  int16_t PointAngle;
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void OLED_ShowFloatNum(uint8_t X, uint8_t Y, double Number, uint8_t IntLength,
  ...
  Number -=
      IntNum; // 将Number的整数减掉，防止之后将小数乘到整数时因数过大造成错误
 ...
 * 参    数：Chinese
 * 指定要显示的汉字串，范围：必须全部为汉字或者全角字符，不要加入任何半角字符
...
void OLED_ShowChinese(uint8_t X, uint8_t Y, char *Chinese) {
  ...
  for (i = 0; Chinese[i] != '\0'; i++) // 遍历汉字串
  {
    ...
    if (pChinese >= OLED_CHN_CHAR_WIDTH) {
      ...
      for (pIndex = 0; !OLED_StringEqual(OLED_CF16x16[pIndex].Index, "");
           pIndex++) {
        /*找到匹配的汉字*/
        if (OLED_StringEqual(OLED_CF16x16[pIndex].Index, SingleChinese)) {
          break; // 跳出循环，此时pIndex的值为指定汉字的索引
        }
      }

      /*将汉字字模库OLED_CF16x16的指定数据以16*16的图像格式显示*/
      OLED_ShowImage(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, Y, 16, 16,
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void OLED_ShowImage(uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height,
                    const uint8_t *Image) {
  uint8_t i, j;
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void OLED_DrawArc(uint8_t X, uint8_t Y, uint8_t Radius, int16_t StartAngle,
