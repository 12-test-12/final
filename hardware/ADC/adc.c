...
void MY_ADC_Init(void) {
  ...
  ADC_InitStructure.ADC_Mode =
      ADC_Mode_Independent; // ADC独立工作模式，不使用多ADC联动
  ADC_InitStructure.ADC_ScanConvMode =
      DISABLE; // 关闭扫描模式，单通道不需要扫描
  ADC_InitStructure.ADC_ContinuousConvMode =
      DISABLE; // 关闭连续转换，每次需要软件手动触发（重点）
  ADC_InitStructure.ADC_ExternalTrigConv =
      ADC_ExternalTrigConv_None; // 无外部触发，使用软件触发
...
float MQ135_EstimatePpm(uint16_t adc_value) {
  ...
  // 结果限幅，防止异常ADC值导致浓度溢出
  if (nh3_conc < 1.0f)
    nh3_conc = 1.0f; // 最低限制1ppm，避免出现0
  if (nh3_conc > 999.0f)
    nh3_conc = 999.0f; // OLED使用3位显示
 ...
 */
float MQ135_GetData(void) { return MQ135_EstimatePpm(ADC_GetAvgValue()); }
