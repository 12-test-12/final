...
| LED | 控制端 | PA4 | 高电平报警 |
| 有源/无源蜂鸣器 | 控制端 | PB13 / TIM1_CH1N | 2 kHz PWM 报警 |
...
1. **已烧录并完成首轮上电验证。** ST-LINK 烧录/校验、OLED、MQ135 ADC、ESP8266 手机热点入网与 MQTT 上报已通过。DHT11 仍返回时序错误，温湿度为 0，需检查 PA5 接线、上拉和传感器型号。
...
7. **MQTT 上行已通，下行尚未完成。** 驱动已支持 640 字节二进制帧，真实遥测已经由 EMQX 进入 Backend 并落库。但控制 PUBLISH 的命令解析、动作和 ACK 还未串入主循环。
