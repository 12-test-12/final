...
static uint8_t ESP8266_MatchStatusText(char received,
                                       const char *text,
...
static void ESP8266_ParseIncomingByte(char received)
{
    ...
    if ((s_ipdState != 6U) &&
        (ESP8266_MatchStatusText(received,
                                 s_wifiGotIpText,
                                 &s_wifiGotIpIndex) != 0U))
    {
        s_wifiConnected = 1U;
...
static uint8_t ESP8266_WaitFor(const char *expected,
...
static uint8_t ESP8266_AppendChar(char *buffer,
                                  uint16_t capacity,
                                  uint16_t *length,
                                  char value)
{
...
static uint8_t ESP8266_AppendString(char *buffer,
                                    uint16_t capacity,
                                    uint16_t *length,
                                    const char *text)
{
...
static uint8_t ESP8266_AppendUnsigned(char *buffer,
                                      uint16_t capacity,
                                      uint16_t *length,
                                      uint16_t value)
{
...
uint8_t ESP8266_Task(void)
{
    ...
    {
        buffer[0] = '\0';
        (void)ESP8266_AppendString(buffer, sizeof(buffer), &length, "AT+CIPSTART=\"TCP\",\"");
