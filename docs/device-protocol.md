...
**设备端分辨率（固件实现约束，非契约放宽）**：DHT11 只有 1 °C 分辨率，气体估算只到整 ppm，因此设备把收到的小数阈值**四舍五入到整度/整 ppm** 后执行（30.5 → 31）。选择四舍五入而不是截断，是为了避免所有阈值被静默下移最多一个单位。`thresholdVersion` 仍然记录"配置已下发并写入 Flash"这一事实；客户端不应假设设备按小数位精确执行。若将来更换为分辨率更高的传感器，该舍入应当重新评估。
